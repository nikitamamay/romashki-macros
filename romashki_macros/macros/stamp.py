"""
Макрос для работой с основной надписью двухмерного документа (чертежа, спецификации).

Макрос предоставляет функционал:
* заполнить основную надпись по шаблону;
* заполнить все ячейки основной надписи их номерами;
* получить содержимое ячейки основной надписи;
* форматировать дату (текущую или заданную) по шаблону.
...
class StampCellNumbers:
    """ Номера ячеек основной надписи первого листа конструкторского чертежа. """
    Naimenovanie = 1
    Oboznachenie = 2
    Material = 3
    Litera0 = 40
...
def format_date(date: datetime.date|None = None, fmt: str = "%d.%m.%y") -> str:
    """
    Форматирует дату по заданному шаблону `fmt` функцией `strftime()`.
    Если объект `date` равен `None`, то используется текущая дата.

    Наиболее частые варианты `fmt`:
