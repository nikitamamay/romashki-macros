...
Целью же данного макроса является именно обозначение сварных швов с помощью
упрощенных твердых тел этих швов и, как следствие, указание тех мест, где
...
Сценарий работы с макросом следующий. Твердые тела сварных швов можно создавать
...
RMWELD = "RMWeld"
""" Префикс по умолчанию для построений """
...
def remove_empty_lines(lines: list[Line]) -> list[Line]:
    ...
    Изменяет сам переданный список `lines`, не_создавая новый.
    """
    i = 0
    while i < len(lines):
        if len(lines[i]) < 2:
...
def create_weld_body(
    ...
    edge_collection: KAPI5.ksEdgeCollection = polyline5.EdgeCollection()
    ...
    edge5: KAPI5.ksEdgeDefinition = edge_collection.First()
