"""
Модуль предоставляет классы виджетов, которые используются, как правило,
...
class LineEditDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def createEditor(
        ...
        model.setData(index, index.data(), self.ROLE_OLD_VALUE)
        return editor
...
class StringListSelector(QtWidgets.QWidget):
    ...
    def __init__(self, create_new_item_func = lambda: QtGui.QStandardItem("элемент"), parent = None) -> None:
        ...
        self._view_selectionModel: QtCore.QItemSelectionModel = self._view.selectionModel(); assert self._view_selectionModel is not None
        self._view_selectionModel.selectionChanged.connect(self._selection_changed)
    ...
    def move_selected_in_order(self, do_move_up: bool) -> None:
        ...
        if (selected_row_indexes[0] > 0 and do_move_up) \
            ...
            self._view_selectionModel.clear()
            for r in selected_row_indexes:
                r2 = r + (-1 if do_move_up else +1)
                self._view_selectionModel.select(
...
class ButtonWithList(QtWidgets.QToolButton):
    def __init__(self, icon: QtGui.QIcon|None, text: str|None, parent = None) -> None:
        super().__init__(parent)
        if text is not None:
            self.setText(text)
        if icon is not None:
    ...
    def menu(self) -> QtWidgets.QMenu:  # указан возвращаемый тип строго как `QMenu`, а не `QMenu|None`
        return self._menu
