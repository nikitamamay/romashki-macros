"""
Модуль графического интерфейса макроса `fast_parts`.

Графический интерфейс макроса позволяет:

* создать out-детали автоматически;
* создать одну out-модель для дальнейшего редактирования вручную;
* ориентировать компоненты;
* вставить компоненты из компоновочной геометрии;
* настраивать:
    * опцию закрытия файлов out-деталей при их автоматическом создании.
