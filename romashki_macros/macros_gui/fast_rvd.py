"""
Модуль графического интерфейса макроса `fast_rvd`.

Графический интерфейс позволяет:
* применить размер РВД для эскиза и операции выдавливания по траектории;
* настраивать список размеров РВД.

Пример использования: пользователь вручную создает 3D-сплайн, который представляет
собой среднюю линию рукава высокого давления; создает эскиз на плоскости фитинга
(так, чтобы и сплайн начинался от этой плоскости); создает на эскизе окружность
любого диаметра, не создает на неё ограничение размера; создает операцию выдавливания
по траектории того 3D-сплайна. Затем выделяет в дереве построения созданный эскиз
и операцию выдавливания по траектории и запускает команду макроса.
Окружность эскиза приобретает наружный диаметр РВД, а стенка операции выдавливания ---
толщину стенки РВД.
"""
