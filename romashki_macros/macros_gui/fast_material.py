"""
Модуль графического интерфейса макроса `fast_material`.

Графический интерфейс позволяет:
* назначить материал (и плотность) в 3D-модели
    * произвольным образом;
    * из собственного списка;
* сохранить материал текущей 3D-модели в список;
* настраивать список наиболее часто используемых материалов;
...
from ..macros.fast_material import *


def ensure_value(value, default):  # FIXME смотри, где используется эта функция. Проблемы с созданием Items ?
    if value is None:
        return default
    return value
...
class MacrosFastMaterial(Macros):
    ...
    def __init__(self) -> None:
        ...
        self._dialog: QtWidgets.QWidget = None
        self._dialog_miw: MaterialInputWidget = None
        self._is_selector_widget_inited: bool = False
