"""
Модуль графического интерфейса макроса `fast_dxf`.

Графический интерфейс позволяет:
* обновить ориентацию главного вида в модели;

    Функция создания ориентации в модели под кодовым названием "Главный вид"
    для последующего создания на чертеже ассоциативного вида по этой ориентации.
    Это актуально для старых версий Компаса, в которых нет возможности изменить
    ориентацию стандартных видов для дальнейшего проецирования в чертеж, а нужно
    создавать новый вид (ориентацию).

* обновить DXF-ориентацию в модели;
* создать DXF для открытой детали;
* создать DXF из открытого чертежа;
* сохранить текущий фрагмент в DXF;
* настраивать:
    * шаблон имени файла DXF при сохранении фрагмента;
    * опцию переименовывания вида чертежа на кодовое слово "DXF".

Принцип работы с макросом:
* Создание DXF из 3D-модели:
    1. пользователь создает ориентацию (вид, проекцию), ортогональную развертке
    детали, для указания направления проецирования;
    2. по команде автоматически создается временный чертеж с ассоциативным видом
    по этой ориентации модели, чтобы получить контуры детали;
    контуры переносятся во фрагмент;
    3. пользователь может редактировать фрагмент, как правило, в целях
    удаления каких-то линии, например, резьбовых отверстий;
    4. по команде происходит сохранение в формате DXF или в формате Компас-Фрагмент
    с указанием в имени файла толщины, обозначения и наименования детали.
* Создание DXF из вида чертежа:
    1. пользователь выделяет вид на чертеже;
    2. по команде создается фрагмент и линии вида переносятся в него;
    3. пользователь может редактировать фрагмент, как правило, в целях
    удаления каких-то линии, например, резьбовых отверстий;
    4. по команде происходит сохранение в формате DXF или в формате Компас-Фрагмент
    с указанием в имени файла толщины, обозначения и наименования детали.
...
class MacrosFastDXF(Macros):
    ...
    def check_config(self) -> None:
        config_reader.ensure_dict_value(self.config(), "do_rename_selected_view_to_DXF", bool, False)
        config_reader.ensure_dict_value(
            self.config(), "filename_template", str,
            f"S={TEMPLATE_KEYWORD_THICKNESS} {TEMPLATE_KEYWORD_MARKING} {TEMPLATE_KEYWORD_NAME}")

    def settings_widget(self) -> QtWidgets.QWidget:
        def _apply_changes():
            self.config()["do_rename_selected_view_to_DXF"] = cb_do_rename_view_in_dwg.isChecked()
            self.config()["filename_template"] = le_filename_fmt.text()
            _show_filename_example()
            config.save_delayed()

        def _show_filename_example():
            fp = get_dxf_path(self.config()["filename_template"], "./", 2.5, "АБВГ.000.001", "Пластина")
        ...
        le_filename_fmt = QtWidgets.QLineEdit(self.config()["filename_template"])
        ...
        cb_do_rename_view_in_dwg.setChecked(self.config()["do_rename_selected_view_to_DXF"])
    ...
    def toolbar_widgets(self) -> dict[str, QtWidgets.QWidget]:
        ...
        btn_dxf_from_part.clicked.connect(lambda: self.execute(lambda: create_DXF_from_part(self.config()["filename_template"])))
        ...
        btn_dxf_from_dwg.clicked.connect(lambda: self.execute(lambda: create_DXF_from_dwg(self.config()["filename_template"], self.config()["do_rename_selected_view_to_DXF"])))
